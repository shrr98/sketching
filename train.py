...
def collect_gameplay_experiences_with_random(env, agent, buffer, ontrain=True):
    """
    ...
    while not done:
        ...
        if (reward == -1 and len([x for x in last_positions if x==curr_position])>=2) or \
...
def collect_gameplay_experiences(env, agent, buffer, ontrain=True):
    ...
    while not done:
        ...
        ## Stuck by position
        curr_position = env.drawer.get_pen_position()
        if (reward == -5 and len([x for x in last_positions if x==curr_position])>=2) or \
            (reward<=0 and len([x for x in last_positions if x==curr_position])>=2):
        # if reward <= -5:
            pos = last_positions.index(curr_position)
            ...
            if False:#np.random.rand() < 0.1:
                actions = [env.get_random_action(pen_state=None)]
            else:
                actions = env.get_actions_to_nearest_stroke()
...
def train_model(max_episodes=50000):
    ...
    for episode_cnt in tqdm(range(1, max_episodes+1), ascii=True, unit = "episode"):
        ...
        if episode_cnt % REDUCE_LR_EVERY == 0:
...
train_model(max_episodes=50000)